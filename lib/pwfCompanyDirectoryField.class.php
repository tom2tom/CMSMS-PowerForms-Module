...
class pwfCompanyDirectoryField extends pwfFieldBase
{
	...
	function GetFieldInput($id, &$params, $returnid)
	{
		...
		$query = 'SELECT com.id, com.company_name FROM '.$pref.'module_compdir_company_categories AS comcat '
				.'LEFT JOIN '.$pref.'module_compdir_categories AS cat ON cat.id = comcat.category_id '
				.'LEFT JOIN '.$pref.'module_compdir_companies AS com ON com.id = comcat.company_id '
				.'WHERE cat.name '.$processPath.' AND status = \'published\'';

		$query2 = 'SELECT value FROM '.$pref.'module_compdir_fieldvals AS comfv '
				.'LEFT JOIN '.$pref.'module_compdir_fielddefs AS fdd ON fdd.id = comfv.fielddef_id '
				.'LEFT JOIN '.$pref.'module_compdir_companies AS com ON comfv.company_id = com.id '
