...
class SaferEval
{
	public $source, $allowedCalls, $allowedTokens, $allowedVariables, $disallowedExpressions;
	...
	public function evalSyntax($code)
	{ // Separate function for checking syntax without breaking the script
	...
	public function checkScript($code, $execute)
	{
		...
		elseif (!$this->evalSyntax($this->code)) {
			$this->errors[0]['name'] = 'Syntax error.';
		}
