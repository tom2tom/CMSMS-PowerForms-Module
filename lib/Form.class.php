...
";
	$jsCode .= "<input type=\"button\" value=\"".
		$button_text."\" onclick=\"javascript:populate_".$fldAlias."(this.form)\" />";
	return $jsCode;
  }
*/
...
*/
  ...
  private function CreateAction($id, $fieldName='opt_email_template', $button_text='', $suffix='')
  {
	...
	$func = "
function populate_".$fldAlias."(formname) {
 if (confirm (\"".$this->module_ptr->Lang('confirm')."\")) {
  formname['".$id."fbrp_".$fieldName."'].value=|TEMPLATE|;
 }
}
";
	$btn = '<input type="button" class="cms_submit" value="'.$button_text.
		'" onclick="javascript:populate_'.$fldAlias.'(this.form)" />';
  ...
  function AdminTemplateActions($formDescriptor,$fieldStruct)
  {
	...
	foreach ($fieldStruct as $key=>$val)
	{
		$html_button = (isset($val['html_button']) && $val['html_button']);
		$text_button = (isset($val['text_button']) && $val['text_button']);
		$gen_button = (isset($val['general_button']) && $val['general_button']);
		$is_oneline = (isset($val['is_oneline']) && $val['is_oneline']);
		$is_email = (isset($val['is_email']) && $val['is_email']);
		$is_header = (isset($val['is_header']) && $val['is_header']);
		$is_footer = (isset($val['is_footer']) && $val['is_footer']);
...
//	$row['name'] .= ' '.$this->module_ptr->Lang('copy');
	...
	if ($neworder === false)
	{
		$sql = 'SELECT MAX(order_by) AS last FROM ' .$pref.
		 'module_fb_field WHERE form_id=?';
	...
	$sql = 'INSERT INTO ' .$pref.
	 'module_fb_field (field_id,form_id,name,type,validation_type,required,hide_label,order_by) '.
	 'VALUES (?,?,?,?,?,?,?,?)';
	$db->Execute ($sql, $row);
  ...
  function Copy()
  {
	...
	$name = $this->GetName();
	if ($name) $name .= ' '.$this->module_ptr->Lang('copy');
	$alias = $this->GetAlias();
	if ($alias) $alias .= '_'.$this->MakeAlias($this->module_ptr->Lang('copy'), true);
  ...
  function AddEditForm($id, $returnid, $tab, $message='')
  {
	...
	$smarty->assign('icon_info',
		$theme->DisplayImage('icons/system/info.gif','true','','','systemicon'));
	...
	if($this->Id > 0)
	{
		...
		foreach ($this->Fields as &$fld)
		{
			...
			{
				$oneset->disposition = $mod->CreateLink($id, 'admin_update_field_required', '', $theme->DisplayImage('icons/system/true.gif','true','','','systemicon'), array('form_id'=>$this->Id,'fbrp_active'=>'off','field_id'=>$fld->GetId()),'', '', '', 'class="true" onclick="jQuery(this).fb_admin_update_field_required(); return false;"');
			}
			else
			{
				$oneset->disposition = $mod->CreateLink($id, 'admin_update_field_required', '', $theme->DisplayImage('icons/system/false.gif','false','','','systemicon'), array('form_id'=>$this->Id,'fbrp_active'=>'on','field_id'=>$fld->GetId()),'', '', '', 'class="false" onclick="jQuery(this).fb_admin_update_field_required(); return false;"');
			}

			$oneset->field_status = $fld->StatusInfo();
			$oneset->editlink = $mod->CreateLink($id, 'admin_add_edit_field', '', $theme->DisplayImage('icons/system/edit.gif',$mod->Lang('edit'),'','','systemicon'), array('field_id'=>$fld->GetId(),'form_id'=>$this->Id));
			$oneset->copylink = $mod->CreateLink($id, 'admin_copy_field', '', $theme->DisplayImage('icons/system/copy.gif',$mod->Lang('copy'),'','','systemicon'), array('field_id'=>$fld->GetId(),'form_id'=>$this->Id));
			$oneset->deletelink = $mod->CreateLink($id, 'admin_delete_field', '', $theme->DisplayImage('icons/system/delete.gif',$mod->Lang('delete'),'','','systemicon'), array('field_id'=>$fld->GetId(),'form_id'=>$this->Id),'', '', '', 'onclick="jQuery(this).fb_delete_field(\''.$mod->Lang('are_you_sure_delete_field',htmlspecialchars($fld->GetName())).'\'); return false;"');

			/* Removed By Stikki, reinstated by SjG with Javascript to hide it if Javascript's enabled. */
			if ($count > 1)
			{
				$oneset->up = $mod->CreateLink($id, 'admin_update_field_order', '', $theme->DisplayImage('icons/system/arrow-u.gif','up','','','systemicon'), array('form_id'=>$this->Id,'fbrp_dir'=>'up','field_id'=>$fld->GetId()));
			}
			else
			{
				$oneset->up = '&nbsp;';
			...
			if ($count < $last)
			{
				$oneset->down=$mod->CreateLink($id, 'admin_update_field_order', '', $theme->DisplayImage('icons/system/arrow-d.gif','down','','','systemicon'), array('form_id'=>$this->Id,'fbrp_dir'=>'down','field_id'=>$fld->GetId()));
			}
			else
			{
				$oneset->down = '&nbsp;';
...
";
			$jsfuncs [] = str_replace('&amp;','&',$typeFunc); //TODO handle duplicates
			$mod->initialize();
