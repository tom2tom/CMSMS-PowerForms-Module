...
class pwfCCEmailAddressField extends pwfFieldBase
{
	function __construct(&$formdata, &$params)
	{
		...
		$this->IsInput = TRUE;
		$this->Type = 'BCCEmailAddressField';
		$this->ValidationType = 'email';
		$this->ValidationTypes = array($formdata->pwfmodule->Lang('validation_email_address')=>'email');
	...
	function GetFieldInput($id, &$params, $returnid)
	{
		...
		return $mod->CustomCreateInputType($id, '_'.$this->Id,
	...
	function PrePopulateAdminForm($formDescriptor)
	{
		...
		foreach($others as &$one)
		{
			if($one->IsDisposition()
				&& is_subclass_of($one,'pwfDispositionEmailBase'))
			{
		...
		$main[] = array($mod->Lang('title_field_to_modify'),
			$mod->CreateInputDropdown($formDescriptor, 'opt_field_to_modify', $fieldlist, -1, $this->GetOption('field_to_modify')));
