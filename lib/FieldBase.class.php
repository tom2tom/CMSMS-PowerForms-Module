...
class fbFieldBase
{
	...
	var $HasLabel=1;
	var $NeedsDiv=1;
	var $SmartyEval; //true for textinput field whose value is to be processed via smarty
	...
	function __construct(&$form_ptr, &$params)
	{
		...
		$this->ValidationTypes = array($form_ptr->module_ptr->Lang('validation_none')=>'none');
		$this->loaded = false;
	...
	/*
	  ...
	  Construct base content for field add/edit. See also - comments below, for
	  PrePopulateAdminForm()

	  Returns: an associative array with keys 'main' and 'adv', for use by the
		relevant PrePopulateAdminForm(), and ultimately by fbForm->AddEditField().
	*/
	...
	/*
	  ...
	  Construct content for specific field add/edit. Override this.
	  ...
	  Returns: an associative array with 0 or more keys recognised by fbForm->AddEditField().
	*/
...
//			$this->Options[$theArray['name']] = $theArray['attributes']['name'];
	...
	{
		$db = $this->form_ptr->module_ptr->dbHandle;
		$pref = cms_db_prefix();
		if($this->Id == -1)
		{
			$this->Id = $db->GenID($pref.'module_fb_field_seq');
			$sql = 'INSERT INTO '.$pref.'module_fb_field (field_id,form_id,name,type,' .
		...
		{
			$sql = 'UPDATE ' .$pref.
			  'module_fb_field SET name=?,type=?,required=?,validation_type=?,order_by=?,hide_label=? WHERE field_id=?';
			$res = $db->Execute($sql,
		...
		{
			// drop old options
			$sql = 'DELETE FROM '.$pref.'module_fb_field_opt where field_id=?';
			...
			foreach($this->Options as $thisOptKey=>$thisOptValueList)
			{
				...
				{
					$optId = $db->GenID($pref.'module_fb_field_opt_seq');
	...
	function Delete()
	{
		...
		$pref = cms_db_prefix();
		$sql = 'DELETE FROM '.$pref.'module_fb_field where field_id=?';
		$res = $this->form_ptr->module_ptr->dbHandle->Execute($sql,array($this->Id));
		$sql = 'DELETE FROM '.$pref.'module_fb_field_opt where field_id=?';
		$res = $this->form_ptr->module_ptr->dbHandle->Execute($sql,array($this->Id)) && $res;
		return $res;
	}
}
