...
/*
	...
	* /
	...
	fields in table $pre.'module_pwf_record'
	...
	* /
	public function InsertData($form_id,$stamp,&$data,&$pass,&$db,$pre)
	{
		...
		'module_pwf_record (form_id,submitted,contents) VALUES (?,?,?)',
			array($form_id,$when,$cont));
	}
	...
	* /
	public function UpdateData($record_id,&$data,&$pass,&$db,$pre)
	{
		...
		'module_pwf_record SET submitted=?,contents=? WHERE record_id=?',
	...
	* /
	public function LoadData($record_id,&$pass,&$mod=NULL,&$db=NULL,$pre='')
	{
		...
		if(!$pre)
			$pre = cms_db_prefix();
		$row = $db->GetRow(
		'SELECT submitted,contents FROM '.$pre.'module_pwf_record WHERE record_id=?',
			array($record_id));
		if($row)
		{
			$formdata = ($pass) ?
		...
		return array(FALSE,$mod->Lang($errkey));
	}
*/
