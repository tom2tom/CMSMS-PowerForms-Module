...
if ($formdata->Fields) {
	foreach ($formdata->Fields as &$obfld) {
		if ($obfld && $obfld->DisplayInSubmission()) {
			$one = new stdClass();
			$t = $obfld->GetAlias(); //NOT ForceAlias()
			if (!$t) {
				$t = $obfld->GetVariableName();
			}
			$one->name = $t.'} / {$fld_'.$obfld->GetId();
...
$jsloads[] = <<<EOS
 ...
 if($('input[name="{$id}fp_submit_action"]:checked').val() == 'redir') {
  $('#tplobjects').hide();
 } else {
  $('#pageobjects').hide();
 }
 $('input[name="{$id}fp_submit_action"]').change(function() {
  if(this.value == 'redir') {
   $('#tplobjects').hide();
   $('#pageobjects').show();
  } else {
